...
export async function POST(request:Request){
  ...
  const db=(env as unknown as{DB:D1Database}).DB;await ensureRuntimeSchema(db);const matched:Array<{target:Target;fixture:any;raw:any;safeContext:FixtureContext|null}>=[];const unmatched=[];for(const target of list){const candidates=fixtures.map(f=>({fixture:f,context:apiContext(f)})).filter((pair):pair is {fixture:any;context:FixtureContext}=>!!pair.context&&sameName(target.goal.home.name,pair.context.home.name)&&sameName(target.goal.away.name,pair.context.away.name));const safe=candidates.filter(pair=>matchFixturePair(target.goal,pair.context).status==='SAFE_MATCH');const chosen=safe.length===1?safe[0].fixture:candidates[0]?.fixture;if(!chosen){unmatched.push(target);continue}const raw=await requestApi('/odds',{fixture:String(chosen.fixture.id)});matched.push({target,fixture:chosen,raw,safeContext:safe.length===1?safe[0].context:null})}
  const createdAt=new Date().toISOString(),runId=`${createdAt}-${crypto.randomUUID()}`;
  // Persist raw collection first. Identity/normalization failures below must never erase it.
  ...
  try{typed=await saveTypedOdds(db,{legacyRunId:runId,capturedAt:createdAt,apiRequests,sources:typedSources,raw:{format:'candidate-odds-dual-write-v1',matched:typedSources.map(source=>({apiFixtureId:source.apiFixtureId,coreFixtureId:source.coreFixtureId}))}})}catch(error){typed.error=1;console.error('typed odds dual-write failed; raw odds remains saved',error)}
  return Response.json({runId,saved:true,apiRequests,matched:matched.length,unmatched,identity,typed});
}
