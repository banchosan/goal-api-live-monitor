...
import { ApiFootballResponseError, apiFootballFailure, parseApiFootballResponse, publicApiFootballFailure } from '@/lib/api-football-response';
...
export async function POST(request:Request){
  ...
  try {
    ...
    const body=await request.json().catch(()=>null);const list=targets(Array.isArray(body?.candidates)?body.candidates:[]);if(!list.length)return Response.json({ok:false,error:'候補試合がありません',apiRequests:0},{status:400});if(list.length>40)return Response.json({ok:false,error:'一度に取得できる候補は40試合までです',apiRequests:0},{status:400});
    let lastRequest=0;
    ...
    const fixtureResponses:any[]=[];const fixtures:any[]=[];const failures:any[]=[];let haltForRateLimit=false;
    ...
    const failure=apiFootballFailure(error,'/candidate-odds');
    const status=failure.status && failure.status>=400 ? failure.status : 500;
    console.error('candidate odds route failed', failure);
