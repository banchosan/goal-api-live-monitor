...
type LiveMatch = Fixture & { stats: Stat[]; updatedAt?: string; lastReceivedAt?: string; updates: number; ended?: boolean; htStats?: Stat[] | null; daCutoffStats?: Stat[] | null; daCutoffMinute?: number | null; koStats?: Stat[] | null; koBaselineMinute?: number | null; koCutoffStats?: Stat[] | null; koCutoffMinute?: number | null; subscriptionState?: string; subscribedAt?: string | null; snapshots: ManualSnapshot[]; selectedSnapshotId?: string };
...
function DangerousAttacksSignalPanel({ match, signals }: { match: LiveMatch; signals: LiveSignal[] }) {
  ...
  const koHome=statValue(match.koStats??[],'Dangerous Attacks','home'); const koAway=statValue(match.koStats??[],'Dangerous Attacks','away');
  ...
  const koIncreaseHome=koHome===null||koCurrentHome===null?null:koCurrentHome-koHome; const koIncreaseAway=koAway===null||koCurrentAway===null?null:koCurrentAway-koAway;
  const koHomeFired=signals.some((signal)=>signal.ruleId==='dangerous_attacks_kickoff_increase'&&signal.ruleVersion==='v1'&&signal.signalSide==='HOME');
  const koAwayFired=signals.some((signal)=>signal.ruleId==='dangerous_attacks_kickoff_increase'&&signal.ruleVersion==='v1'&&signal.signalSide==='AWAY');
  const koHomeHot=koHomeFired || (koIncreaseHome !== null && koIncreaseHome >= 20); const koAwayHot=koAwayFired || (koIncreaseAway !== null && koIncreaseAway >= 20);
  const koReady=koHome!==null&&koAway!==null; const koCheckpoint=match.koCutoffMinute??null;
  const koState=minute!==null&&minute>25?'判定終了':!koReady?(minute!==null&&minute>1?'KO未取得 / 判定不可':'KO待ち'):'監視中';
  return <><div className="snapshot-delta da-signal-panel"><div className="delta-title"><b>KO → 25' DANGEROUS ATTACKS SIGNAL</b><span>threshold +20</span></div><div className="stat-head"><span>HOME</span><b>{koCheckpoint===null?'checkpoint待ち':`${koCheckpoint}'まで`}</b><span>AWAY</span></div><div className="stat-row"><strong>{koHome===null?'—':koHome}</strong><span>KO DA</span><strong>{koAway===null?'—':koAway}</strong></div><div className="stat-row"><strong>{koCurrentHome===null?'—':koCurrentHome}</strong><span>DA（25分まで）</span><strong>{koCurrentAway===null?'—':koCurrentAway}</strong></div><div className="stat-row delta-row"><strong className={koHomeHot?'signal-fired':''}>{!koReady?'—':koIncreaseHome===null?'—':`${koHomeHot?'🔥 ':''}${signed(koIncreaseHome)}`}</strong><span>{koHomeHot?'20到達・発火':'increase / +20'}</span><strong className={koAwayHot?'signal-fired':''}>{!koReady?'—':koIncreaseAway===null?'—':`${koAwayHot?'🔥 ':''}${signed(koIncreaseAway)}`}</strong></div><small>Status: {koState}{minute!==null&&minute>25?'（表示値は25分以下の最終Socket snapshot）':koReady?` / HOME ${koHomeHot?'🔥 発火':'未発火'} / AWAY ${koAwayHot?'🔥 発火':'未発火'}`:''}</small></div><div className="snapshot-delta da-signal-panel"><div className="delta-title"><b>HT → 65' DANGEROUS ATTACKS SIGNAL</b><span>threshold +15</span></div><div className="stat-head"><span>HOME</span><b>{checkpointMinute===null?'checkpoint待ち':`${checkpointMinute}'まで`}</b><span>AWAY</span></div><div className="stat-row"><strong>{htHome===null?'—':htHome}</strong><span>HT DA</span><strong>{htAway===null?'—':htAway}</strong></div><div className="stat-row"><strong>{currentHome===null?'—':currentHome}</strong><span>DA（65分まで）</span><strong>{currentAway===null?'—':currentAway}</strong></div><div className="stat-row delta-row"><strong className={homeHot?'signal-fired':''}>{!baselineReady?'—':homeIncrease===null?'—':`${homeHot?'🔥 ':''}${signed(homeIncrease)}`}</strong><span>{homeHot?'15到達・発火':'increase / +15'}</span><strong className={awayHot?'signal-fired':''}>{!baselineReady?'—':awayIncrease===null?'—':`${awayHot?'🔥 ':''}${signed(awayIncrease)}`}</strong></div><small>Status: {state}{minute!==null&&minute>65?'（表示値は65分以下の最終Socket snapshot）':baselineReady?` / HOME ${homeHot?'🔥 発火':'未発火'} / AWAY ${awayHot?'🔥 発火':'未発火'}`:''}</small></div></>;
}
