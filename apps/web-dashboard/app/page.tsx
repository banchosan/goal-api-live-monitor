...
type LateIntervalKey = 'ht-65' | '65-70' | '70-75' | '75-80';

function LateMatchComparisonPanel({ match }: { match: LiveMatch }) {
  const [selected, setSelected] = useState<LateIntervalKey>('ht-65');
  const intervals: Record<LateIntervalKey, { label: string; startLabel: string; endLabel: string; startMinute: number | null; endMinute: number | null; startStats: Stat[] | null; endStats: Stat[] | null; requiresHt?: boolean }> = {
    'ht-65': { label: 'HT → 65′', startLabel: 'actual HT', endLabel: '65′', startMinute: null, endMinute: match.minute65 ?? match.daCutoffMinute ?? null, startStats: match.htStats ?? null, endStats: match.minute65Stats ?? match.daCutoffStats ?? null, requiresHt: true },
  ...
  const rows = ['Dangerous Attacks', 'On Target', 'Off Target', 'Corners', 'Attacks', 'Ball Possession'].map((type) => ({ type, home: statValue(interval.endStats ?? [], type, 'home'), away: statValue(interval.endStats ?? [], type, 'away'), baseHome: statValue(interval.startStats ?? [], type, 'home'), baseAway: statValue(interval.startStats ?? [], type, 'away') }));
  return <details className="snapshot-delta interval-comparison-panel"><summary><b>LATE MATCH STAT COMPARISON</b><span>実際のSocket checkpoint比較</span></summary><div className="interval-tabs" role="tablist" aria-label="後半区間の比較選択">{(Object.keys(intervals) as LateIntervalKey[]).map((key) => <button type="button" role="tab" aria-selected={selected === key} className={selected === key ? 'active' : ''} onClick={() => setSelected(key)} key={key}>{intervals[key].label}</button>)}</div><div className="stat-head"><span>HOME ±</span><b>{ready ? `${interval.startMinute === null ? interval.startLabel : `${interval.startMinute}'`} → ${interval.endMinute}'` : `${interval.startLabel} → ${interval.endLabel} 待ち`}</b><span>AWAY ±</span></div>{ready ? <>{rows.map((row) => <SignalStatRow key={row.type} label={row.type === 'Ball Possession' ? 'Ball Possession（変化）' : row.type} home={row.home} away={row.away} baseHome={row.baseHome} baseAway={row.baseAway} />)}<small className="interval-note">表示値は各target以前の最新Socket更新です。NULLは比較せず「—」のまま表示します。</small></> : <p className="no-delta">{interval.requiresHt && !interval.startStats?.length ? 'actual HT未取得のため、HT比較は作りません。' : `${interval.startLabel} と ${interval.endLabel} の両方のcheckpointを待っています。`}</p>}</details>;
}
