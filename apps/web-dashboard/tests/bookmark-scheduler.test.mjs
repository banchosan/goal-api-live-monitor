...
test('20枠超過はkickoff順でqueueする',async()=>{const current=Array.from({length:19},(_,i)=>({id:`m${i}`}));const s=setup([row('late','2026-09-07T12:02:30Z'),row('early','2026-09-07T12:01:00Z')],current);const result=await s.scheduler.tick();assert.deepEqual(result.added,['early']);assert.equal(result.queued,1)});
test('30 AUTO_FORM fixtures use the shared 20-slot queue',async()=>{const rows=Array.from({length:30},(_,i)=>({...row(`a${i}`,new Date(Date.parse('2026-09-07T11:00:00Z')+i*60_000).toISOString()),monitorSource:'auto_form'}));const s=setup(rows);const result=await s.scheduler.tick();assert.equal(result.added.length,20);assert.equal(result.queued,10);assert.equal(s.state().fixtures.every(x=>x.monitorSource==='auto_form'),true)});
